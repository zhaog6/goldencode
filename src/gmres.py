"""Fast restarted GMRES method for solving linear systems"""
...
def gmres(A, b, x0=None, tol=1e-5, restart=None, maxiter=None, M=None,
          ...
          version=None, show=False):

    # Faster restarted GMRES based on modified Gram-Schmidt orthogonalization
    if version == 'fast':
        x, info = _gmres_fast(A, b, x0=x0, restart=restart, tol=tol,
                             maxiter=maxiter, M=M, callback=callback,
                             atol=atol, callback_type=callback_type, show=show)
        return x, info

    # Remark: The original code was written by Nathan Bell, Pauli Virtanen,
    # Ralf Gommers et. al., which will be omitted and skipped in the file.
    # The details of the original GMRES implementation can refer to
    # https://github.com/scipy/scipy
    # ..., ...
    # ..., ...


def _gmres_fast(A, b, x0=None, restart=None, tol=1e-5, maxiter=None, M=None,
    ...
    iteration to solve ``Ax = b``. This version is faster than the old version.
